...
class TestStewart(unittest.TestCase):
    ...
    def test_read(self):
        ...
        self.mock.return_value.read.return_value = bytes([0x55, 0xBE, 0x32, 0x0, 0x3F, 0x0, 0x55, 0x0, 0x75, 0x0, 0xF4, 0x2, 0x89, 0xE, 0x64, 0x0, 0xC8, 0x0, 0x2C, 0x1, 0x3D, 0xD, 0x58, 0x98])
    
        self.stw.read()

        self.assertEqual(self.stw.position, [50,63,85,117,756,3721])
        self.assertEqual(self.stw.imu, [100,200,300])
