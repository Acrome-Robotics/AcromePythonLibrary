...
class TestStewart(unittest.TestCase):
    ...
    def test_read(self):
        ...
        self.mock.return_value.read.return_value = bytes([0x55, 0xBE, 0x32, 0x0, 0x3F, 0x0, 0x55, 0x0, 0x75, 0x0, 0xF4, 0x2, 0x89, 0xE, 0x9A, 0x99, 0xCC, 0x42, 0x9A, 0x19, 0x59, 0x43, 0x33, 0xF3, 0xB3, 0x43, 0x9A, 0x99, 0x1A, 0x43, 0x9A, 0x99, 0xAC, 0xC2, 0x5C, 0x8F, 0x5A, 0x41, 0x00, 0x00, 0x20, 0x3F, 0x2B, 0x87, 0x96, 0x3F, 0xAE, 0x47, 0x75, 0xC1, 0xAC, 0xAF, 0x10, 0xCF])
    
        self.dev._read()

        self.assertEqual(self.dev.position, [50,63,85,117,756,3721])
        for i, val in enumerate([102.3, 217.1, 359.9, 154.6, -86.3, 13.66, 0.625, 1.176, -15.33]):
